...
| Inference | `run_pc`, `run_pcalm`, `infer_for_schedule` and `_solve_inner`, looping with `jax.lax.scan` | One function, `relax` (`pcalm/inference.py`), with an ordinary Python loop | PC is the special case of PC-ALM in which the duals never change, so one function covers both. |
